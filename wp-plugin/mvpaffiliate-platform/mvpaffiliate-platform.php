...
// (creatorUserId|origin|ts with the site's proxy_secret), a honeypot field, and
// a server-side IP rate limit on /api/brand-inquiry — which also requires the
// brand's name + email so the creator can always reply. No captcha widget
// (removed v1.0.68 — Turnstile can't cover unbounded creator blog domains).
...
if (!function_exists('mvp_affiliate_render_brand_cta')) {
    function mvp_affiliate_render_brand_cta() {
        ...
        // Captcha removed from the brand modal (v1.0.68). The form is protected by
        // the honeypot + HMAC signature + server-side IP rate limit instead — no
        // widget, so it works on every creator's blog domain with no config.
