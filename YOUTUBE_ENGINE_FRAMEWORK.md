...
- ✅ **Track B (shipped)** — best-in-class image system, modelled on a competitor teardown (they use Nano Banana default / Ideogram, a captured `videoStill`, `subjectImage`, and per-URL scoring — confirming this exact stack):
  - **Nano Banana primary path** (`lib/thumbnail-generators.ts` → `fal-ai/nano-banana/edit`) — composes creator + product + a real video frame into a FINISHED thumbnail in ONE call, preserving identity natively. Replaces the slow gpt-image cut-out → rembg → client-composite chain (the speed win toward ~20s). Returns `composited:true` + `personCutoutUrl:null`; fires `variantCount` parallel composes. Falls through to Kontext/Flux on empty output.
  - **Key-frame grounding** — `lib/youtube-frames.ts` `resolveGroundingFrames` feeds the real hero/mid frames (img.youtube.com, no ffmpeg) as Nano Banana references; route accepts `youtubeVideoId`, studio passes it.
  - **Ideogram v3 path** (`fal-ai/ideogram/v3`) — replaces Flux Pro as the no-reference text-forward generator (cleaner graphic thumbnails / typography).
  - **N-variant (1–10)** — server cap raised from 2; monthly cap pre-flight still budgets the full count. Cost rows added (`fal-nano-banana` $0.039, `fal-ideogram-v3` $0.06).
- ◻️ **Track B follow-ups** — UI multi-variant picker (studio still sends variantCount:1), surface scores/`belowThreshold` in the UI, async fire-and-poll so a slow model still *feels* instant.
