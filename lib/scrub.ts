 ...
 */
const CLAIM_VERBS =
  /\b(?:cure[sd]?|heal(?:s|ed|ing)?|treat(?:s|ed|ing|ment)?|remed(?:y|ies)|stop(?:s|ped|ping)?|prevent(?:s|ed|ing)?|reverse[sd]?|eliminat(?:e|es|ed|ing)|gets?\s+rid\s+of|clear(?:s|ed)?\s+up|fix(?:es|ed)?|banish(?:es|ed)?|fight(?:s|ing)?|kill(?:s|ed|ing)?|relieve[sd]?|relief|sooth(?:e|es|ed|ing)|reduc(?:e|es|ed|ing)|shrink(?:s)?|dissolve[sd]?|boost(?:s|ed|ing)?|detox(?:es|ify|ifies)?|regrow(?:s|th)?|restore[sd]?|repair(?:s|ed)?|support(?:s|ed|ing)?|promot(?:e|es|ed|ing)|improv(?:e|es|ed|ing)|increas(?:e|es|ed|ing)|enhanc(?:e|es|ed|ing)|aids?|help(?:s|ed)?\s+with|maintain(?:s|ed)?|optimi[sz](?:e|es|ed|ing))\b/i

const HEALTH_SUBJECTS =
  /\b(?:cold\s+sores?|herpes|acne|pimples?|blackheads?|eczema|psoriasis|rosacea|dermatitis|warts?|fungus|fungal|toenail\s+fungus|dandruff|hair\s*loss|balding|alopecia|wrinkles?|cellulite|stretch\s+marks?|scars?|pain|aches?|migraines?|headaches?|arthritis|inflammation|swelling|anxiety|depression|stress|insomnia|sleep\s+apnea|snoring|adhd|autism|diabetes|blood\s+sugar|blood\s+pressure|cholesterol|cancer|tumou?rs?|covid|flu|colds?|allergies|asthma|ibs|bloating|constipation|diarrh(?:o)?ea|acid\s+reflux|heartburn|ulcers?|uti|yeast\s+infections?|menopause|cramps?|hangovers?|nausea|vertigo|tinnitus|dry\s+eyes?|varicose\s+veins?|hemorrhoids?|haemorrhoids?|immune\s+system|immunity|metabolism|libido|erectile|fertility|weight\s+loss|belly\s+fat|toxins?|energy\s+levels?|(?:your|more|natural|daily|all[-\s]day|sustained)\s+energy|stamina|endurance|testosterone|hormon(?:es?|al)|cortisol|muscle\s+(?:growth|gain|mass|recovery)|workout\s+recovery|joint\s+(?:health|pain|support|comfort|mobility)|digestion|digestive\s+health|gut\s+health|bone\s+density|brain\s+fog|mental\s+clarity|sleep\s+quality|liver\s+(?:health|function)|kidney\s+(?:health|function)|heart\s+health|circulation|collagen\s+production|nutrient\s+absorption)\b/i
