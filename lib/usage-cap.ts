...
export const PRIMARY_FEATURE = {
  /** Thumbnail route runs exactly ONE primary path per variant —
   *  Kontext (product-only), flux-lora (face), or the Flux Pro fallback
   *  — so summing these three per-image feature names equals total
   *  successful thumbnail generations. NOTE: the face path's stage-2
