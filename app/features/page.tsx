...
const GROUPS: { id: string; n: string; tag: string; head: string; intro: string; items: Feat[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: <Rocket size={20} />, tag: 'Pro', title: 'Launchpad', desc: "Start from a video that isn't on YouTube yet. Upload it once, add a CTA, and MVP takes it to YouTube with the full Co-Pilot finish, then straight to every Amazon storefront your product sells in, with each market's title localized and the audio dubbed where English is not the language." },
  ...
  return (
    <div style={{ ...LIGHT_VARS, backgroundColor: 'var(--bg)', color: 'var(--text)' }} className="min-h-screen font-[Inter,system-ui,sans-serif]">
      ...
      <section className="max-w-3xl mx-auto px-6 py-24 text-center">
        ...
        <p className="mt-6 text-[13.5px] inline-flex items-center gap-1.5" style={{ color: 'var(--text-faint)' }}><Zap size={13} /> {GUARANTEE_LABEL}</p>
      </section>
