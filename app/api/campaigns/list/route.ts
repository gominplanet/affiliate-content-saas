...
export async function GET() {
  ...
  const [{ data, error }, { data: intg }, { data: brand }] = await Promise.all([
    ...
    supabase
      .from('campaigns')
      .select('id,asin,cc_campaign_id,product_title,campaign_name,epc,program,commission_pct,monthly_sales,has_carousel_video,carousel_video_pos,details_url,ends_at,status,error_message,wordpress_url,blog_post_id,category,hero_kind,product_price,created_at')
      .eq('user_id', user.id)
