...
import { personalTagStats, pooledTagStats, collectedSampleCount } from '@/lib/pulse-rank'
...
export async function GET() {
  ...
  const personal = sampleCount >= MIN_FOR_PERSONAL ? await personalTagStats(user.id) : []
  const pooled = await Promise.all(topNiches.map(async (niche) => ({
    niche,
