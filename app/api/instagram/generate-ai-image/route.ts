    ...
    if (igFaceRefs.length > 0) {
      try {
        ...
        const igProductClause = igProductRef
          ? `The FINAL reference image is the PRODUCT — render the ACTUAL PRODUCT ITEM ITSELF (true shape, colour, materials). CRITICAL: if that reference is retail PACKAGING, a box, a poly-bag or a marketing infographic (Amazon A+ Content style with overlay headlines, checkmark badges, callout circles, comparison panels, arrows pointing at features), depict the REAL unpackaged product — NOT the box, NOT the infographic — and do NOT reproduce ANY printed marketing text, feature lists, claims, percentages, ratings, warranty/award badges, size charts, checkmarks, callout pills or composition/layout from the reference. Do NOT copy the reference's framing or staging. The product surface must be clean. Use the reference ONLY to learn what the product physically looks like, never for composition, never for a person.`
          : `Render the ACTUAL product item accurately and prominently as a clean hero object — never its retail box or marketing-infographic packaging, and with no printed claims, feature text, badges or callouts on it.`
