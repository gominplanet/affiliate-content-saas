...
CRITICAL — IGNORE all overlay graphics on the reference: the reference may be an Amazon listing or A+ Content infographic with headlines like "Ultimate ___" or "Premium ___", checkmark badges, circle callouts saying things like "Non-Slip" or "Stable Frame", feature-highlight pills, arrows pointing at product parts, side-by-side panels, or any marketing text/graphic overlay. DO NOT reproduce ANY of those elements. DO NOT copy the reference's composition, layout, framing, or staging. The output must contain ZERO overlay text, ZERO badges, ZERO callouts, ZERO marketing graphics. Strip ALL of that away and use ONLY the physical product itself.

CHANGE the entire scene: brand-new background, brand-new context, brand-new lighting, brand-new camera angle and distance, brand-new product position/orientation. The product is the ONLY thing that carries over from the reference.

Render as a polished magazine-quality editorial photo shown as a ${perspective}${scene ? `: ${scene}` : ', set naturally in a real-world setting that fits how it is actually used'}. If a realistic setting truly doesn't fit, stage the unchanged product on a clean surface against a VIBRANT colour-pop / gradient background with soft studio lighting, reflections, and depth.
...
${NO_BRAND_IMAGE_CLAUSE} Landscape 4:3, photorealistic editorial product photography, no added text/captions/watermarks/badges/callouts/labels.`
        ...
        if (url) {
          ...
          if (!v.match) {
            const stricter = `IDENTITY-LOCKED render. The reference image is the GROUND TRUTH for what "${productTitle}" PHYSICALLY looks like — nothing else. The previous attempt rendered a DIFFERENT product, which is wrong. Copy the product from the reference EXACTLY — same shape, same colour, same cut-out / texture / pattern, same number of components, same on-product branding/text. Do NOT substitute a similar-looking product.

STRIP any overlay text, headlines, checkmark badges, callout circles, feature pills, comparison panels, or marketing graphics that may be on the reference — those are NOT part of the product, do not reproduce them.

Place the product in a COMPLETELY NEW real-world scene — do NOT copy the reference's composition/pose/framing. Vary the angle and position. Background${scene ? `: ${scene}` : ''}.
