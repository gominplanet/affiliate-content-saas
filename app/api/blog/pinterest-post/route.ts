...
export async function POST(request: NextRequest) {
  ...
  if (!tierAllowsSocial(tier, 'pinterest')) {
    return NextResponse.json(
      { error: 'Pinterest auto-publish is on the Amazon and Pro plans. Upgrade to pin to Pinterest.' },
      { status: 403 },
    )
  }
  ...
  const [{ data: post }, { data: integration }] = await Promise.all([
    supabase.from('blog_posts').select('id,user_id,title,wordpress_url,wordpress_post_id,wordpress_site_id,social_publish_counts,video_id,content').eq('id', postId).eq('user_id', user.id).maybeSingle(),
