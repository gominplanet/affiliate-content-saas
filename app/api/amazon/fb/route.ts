...
//
// POST /api/amazon/fb — publish (or schedule) a Facebook Page post from an MVP
// Art Director design + a product. The geni.us affiliate link goes inline in the
// caption.
//
...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => ({})) as {
    imageUrl?: string; productUrl?: string; asin?: string; productTitle?: string; caption?: string; scheduledAt?: string; useShowcase?: boolean; showcaseUrl?: string
  }
  ...
  if (!tierAllowsSocial(tier, 'facebook')) {
    ...
    return NextResponse.json({
      error: 'Facebook posting is on the Amazon and Pro plans.',
  ...
  if (when) {
    ...
    const { data, error } = await (supabase as any)
      ...
      .insert({
        user_id: user.id, platform: 'facebook', image_url: body.imageUrl,
