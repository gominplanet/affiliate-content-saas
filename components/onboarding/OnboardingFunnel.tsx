...
export default function OnboardingFunnel({
  ...
  // Inline support — onboarding has no sidebar, so a stuck new user would have
  // no way to reach us. This opens a help ticket right here: it lands in the
  // admin inbox (+ email/Discord alert) so we can jump in and assist.
  const [helpOpen, setHelpOpen] = useState(false)
  const [helpSubject, setHelpSubject] = useState('')
  const [helpBody, setHelpBody] = useState('')
  const [helpSending, setHelpSending] = useState(false)
  const sendHelp = useCallback(async () => {
    const subject = helpSubject.trim(), body = helpBody.trim()
    if (!subject || !body) { toast.error('Add a subject and a short message.'); return }
    setHelpSending(true)
    try {
  ...
  return (
    <div className="min-h-screen w-full text-[#f5f5f7]" style={{ background: 'radial-gradient(1200px 600px at 50% -10%, rgba(124,58,237,0.18), transparent), #0a0a0f' }}>
      <div className="mx-auto max-w-5xl px-5 py-8 md:py-12">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs text-[#a1a1a6] hover:text-white transition-colors"
            >
        ...
        {/* Inline support ticket — reachable from any step so a stuck new user
            can always reach us; the ticket opens in the admin inbox. */}
        {helpOpen && (
          <div
            className="fixed inset-0 z-50 flex items-center justify-center p-4"
            style={{ background: 'rgba(0,0,0,0.6)' }}
            onClick={() => { if (!helpSending) setHelpOpen(false) }}
          >
            <div
              className="w-full max-w-md rounded-2xl p-5"
              style={{ background: '#141419', border: '1px solid rgba(255,255,255,0.1)' }}
            ...
            >
              ...
              <p className="text-xs text-[#a1a1a6] mb-4">
                Stuck on a step? Send us a message and we’ll help you get set up. We’ll reply to <b className="text-[#f5f5f7]">{email}</b>.
              </p>
              <input
                value={helpSubject}
              ...
              <button
                ...
                className="w-full mt-3 inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-semibold text-white disabled:opacity-60"
                style={{ background: ACCENT }}
              >
                {helpSending ? <><Loader2 size={15} className="animate-spin" /> Sending…</> : 'Send message'}
              </button>
            </div>
          </div>
        )}
