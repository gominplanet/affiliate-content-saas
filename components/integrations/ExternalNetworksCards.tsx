...
//
// The affiliate-network connection cards (Levanta, PartnerBoost, Wayward). Pulled
// out of the External Integrations page so the same setup can be surfaced in more
// than one place (the page itself + the Passport Links page). Self-contained:
// fetches status, handles the paid-plan gate, and lets the user paste / replace /
// remove each key. Keys are encrypted server-side; only a masked last-4 is shown.

'use client'

import { useCallback, useEffect, useState } from 'react'
import { ShoppingBag, Store, ExternalLink, Loader2, CheckCircle2, Lock, Trash2 } from 'lucide-react'
...
export default function ExternalNetworksCards() {
  ...
  if (forbidden) {
    return (
      <div className="rounded-xl border p-4 flex items-start gap-3"
        ...
        <p className="text-[13px]" style={{ color: 'var(--text)' }}>Affiliate network connections are available on <strong>paid plans</strong>.</p>
      </div>
    )
  }
