...
import { requestStudioFinish, getScoutStatus, liftoffDone } from '@/lib/extension-frame'
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!r.ok) { say(`Could not read your batches: ${j?.error || r.status}`); await liftoffDone(true, 'error'); return }
        ...
        for (const b of launched) {
          ...
          if (!dr.ok || !d?.ok) { say(`${b.name}: could not be read`); more = true; continue }
          ...
          const before = liftoffPending(items, markets, { sendToYouTube, studioPossible })
          ...
          if (sendToYouTube && studioPossible) {
            for (const it of items) {
              if (!it.youtube_video_id) continue
              if (it.studio_finish && it.studio_finish.error !== 'timeout') continue
              ...
              await fetch(`/api/launch/items/${it.id}`, {
          ...
          const after = liftoffPending((a.items ?? []) as RunnerItem[], markets, { sendToYouTube, studioPossible })
