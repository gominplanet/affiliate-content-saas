    ...
    try {
      ...
      if (res.loaded === 0 || res.diag) {
        try {
          ...
          if (Array.isArray(res.diag?.parts)) parts.push(`parts: ${res.diag.parts.map((p: { q: string; added: number; stop: string }) => `${p.q}=${p.added}(${p.stop})`).join(', ')}`)
          if (res.diag?.stopLoaded != null) parts.push(`loaded: ${res.diag.stopLoaded}`)
