...
/** A live Affiliate Boost promotion for one Walmart item (boosted commission). */
export interface PBWalmartBoost {
  itemId: string
  sku: string | null
  brandName: string | null
  commissionPct: number | null // parsed from the fraction "0.15" → 15
  startTime: string | null
  endTime: string | null
}

/**
 * Pull the current Walmart Affiliate Boost promotions (commission bumps).
 * POST /datafeed/get_latest_affiliate_boost_for_walmart_items { page_size, page }.
 * Returns item-level rows only — enrich via getWalmartProductsByItemIds.
 */
export async function getWalmartAffiliateBoosts(
  token: string,
  opts: { page?: number; pageSize?: number } = {},
): Promise<{ boosts: PBWalmartBoost[]; hasMore: boolean }> {
  const json = await pbPost('/datafeed/get_latest_affiliate_boost_for_walmart_items', token, {
    page: opts.page ?? 1,
    page_size: Math.min(Math.max(opts.pageSize ?? 50, 1), 50),
  })
  if (json?.status?.code !== 0) {
    throw new Error(json?.status?.msg ? `PartnerBoost: ${json.status.msg}` : 'PartnerBoost Affiliate Boost error')
  }
  const data = json?.data || {}
  const list = Array.isArray(data.list) ? data.list : []
  // Boost commission arrives as a FRACTION string ("0.15" = 15%). Coerce to a
  // percent; tolerate it already being a percent ("15" / "15%") defensively.
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const boosts: PBWalmartBoost[] = list.map((b: any): PBWalmartBoost => ({
    itemId: asStr(b.item_id ?? b.itemId ?? b.id),
    sku: asStrOrNull(b.sku),
    brandName: asStrOrNull(b.brand_name ?? b.brandName ?? b.merchant_name),
    commissionPct: toPct(b.commission ?? b.commission_rate ?? b.boost_commission),
    startTime: asStrOrNull(b.start_time ?? b.startTime ?? b.start_date),
    endTime: asStrOrNull(b.end_time ?? b.endTime ?? b.end_date),
  })).filter((b: PBWalmartBoost) => !!b.itemId)
  return { boosts, hasMore: !!(data.has_more ?? data.hasMore) }
}

/**
 * Look up full Walmart products by item id (≤50 per call).
 * POST /walmart_datafeed/get_products { item_id: "a,b,c", page_size, page }.
 */
export async function getWalmartProductsByItemIds(
  token: string,
  itemIds: string[],
): Promise<PBProduct[]> {
  const ids = itemIds.filter(Boolean).slice(0, 50)
  if (ids.length === 0) return []
  const json = await pbPost('/walmart_datafeed/get_products', token, {
    item_id: ids.join(','),
    page_size: 50,
    page: 1,
  })
  ...
  const list = Array.isArray(data.list) ? data.list : []
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return list.map((p: any): PBProduct => {
    ...
    return {
      ...
      url: asStr(p.url ?? p.product_url ?? p.link ?? (p.item_id ? `https://www.walmart.com/ip/${p.item_id}` : '')),
      category: asStrOrNull(p.category ?? p.category_name),
  ...
  }).filter((p: PBProduct) => !!p.name)
}

/**
 * Mint commissionable Walmart tracking links by item id (≤50 per call).
 * POST /walmart_datafeed/get_products_link { item_id: "a,b,c" }.
 * Returns a map item_id → tracking url (best-effort; empty when none minted).
 */
export async function getWalmartProductLinks(
  ...
  if (json?.status?.code !== 0) return {} // best-effort — a link failure must not sink the feed
  const data = json?.data || {}
  ...
  for (const row of list as any[]) {
    ...
    const url = asStr(row?.tracking_url ?? row?.short_link ?? row?.link ?? row?.url ?? row?.attribution_link)
    if (id && url) out[id] = url
  }
  // Some payloads return a plain object map { "<item_id>": "<url>" } instead of a list.
  if (Object.keys(out).length === 0 && data && typeof data === 'object' && !Array.isArray(data.list)) {
    for (const [k, v] of Object.entries(data)) {
      if (k === 'has_more') continue
      if (typeof v === 'string' && v) out[k] = v
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      else if (v && typeof v === 'object') { const s = asStr((v as any).tracking_url ?? (v as any).link ?? (v as any).url); if (s) out[k] = s }
    }
  }
  return out
}

/** A Walmart deal = a boosted product enriched for display + one-click posting. */
export interface PBWalmartDeal extends PBProduct {
  itemId: string
...
/**
 * The "Walmart Deals" feed: current Affiliate Boost promotions, each enriched
 * with product detail + a commissionable tracking link. One boost page (≤50)
 * costs three REST calls (boosts → products-by-item → links-by-item).
 */
export async function getWalmartDeals(
  token: string,
  opts: { page?: number; pageSize?: number } = {},
): Promise<{ deals: PBWalmartDeal[]; hasMore: boolean }> {
  const { boosts, hasMore } = await getWalmartAffiliateBoosts(token, opts)
  if (boosts.length === 0) return { deals: [], hasMore }

  const ids = boosts.map((b) => b.itemId)
  const [products, links] = await Promise.all([
    getWalmartProductsByItemIds(token, ids).catch(() => [] as PBProduct[]),
    getWalmartProductLinks(token, ids).catch(() => ({} as Record<string, string>)),
  ])
  const bySku = new Map<string, PBProduct>()
  for (const p of products) if (p.sku) bySku.set(String(p.sku), p)

  const deals: PBWalmartDeal[] = []
  for (const b of boosts) {
    const prod = bySku.get(b.itemId)
    if (!prod || !prod.name || !prod.url) continue // can't display/post without detail
    const tracking = links[b.itemId] || prod.trackingUrl || ''
    deals.push({
      ...prod,
      trackingUrl: tracking,
      brand: prod.brand ?? b.brandName ?? prod.merchantName,
      itemId: b.itemId,
      boostCommissionPct: b.commissionPct,
      startTime: b.startTime,
      endTime: b.endTime,
    })
  }
  // Best boost first.
  deals.sort((a, b) => (b.boostCommissionPct ?? 0) - (a.boostCommissionPct ?? 0))
